...
This package exposes the equivalent `asgrep` and `ast-sgrep` commands and selects one exact-version native package for the current host. Supported hosts at `1.3.1` are macOS arm64/x64, glibc Linux arm64/x64, and Windows x64. Alpine/musl Linux, Windows arm64, and other hosts are rejected explicitly. Installation and execution never download a binary, build Rust source, or search `PATH`.
