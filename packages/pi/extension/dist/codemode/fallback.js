...
const editTails = new Map();
function assertUnicode(text) {
    if (Buffer.from(text, "utf8").toString("utf8") !== text)
        throw new Error("invalid Unicode: unpaired UTF-16 surrogate");
}
...
function clampLine(line) {
    if (charCount(line) <= MAX_LINE_CHARS)
...
async function jailRoot(configuredCwd, args) {
...
async function jailPath(root, raw) {
    assertUnicode(raw);
    if (raw.split(sep === "\\" ? /[\\/]/u : "/").includes(".."))
    ...
    if (!pathContained(root, abs))
        throw new Error(`path escapes session root: ${raw}`);
    return { abs, display: relative(root, abs).split(sep).join("/") };
}
function parseU32Strict(text) {
...
function parseRefString(raw) {
    ...
    if (hash === -1)
        return { path: raw, start: 1, end: 40 };
    ...
    if (start === undefined)
        throw new Error(`invalid ref start in ${raw}`);
    const end = parseU32Strict(endText);
    if (end === undefined)
        throw new Error(`invalid ref end in ${raw}`);
    if (start === 0 || end < start)
        throw new Error(`invalid ref range in ${raw}`);
    return { path: raw.slice(0, hash), start, end };
}
function parseRefValue(value) {
    if (typeof value === "string")
        return parseRefString(value);
    if (value === null || typeof value !== "object" || Array.isArray(value))
        throw new Error("ref must be a string or object");
    const obj = value;
    if (typeof obj.ref === "string")
        return parseRefString(obj.ref);
...
function collectRefs(args) {
    if (Array.isArray(args.refs))
        return args.refs.map(parseRefValue);
    if (args.ref !== undefined)
        return [parseRefValue(args.ref)];
...
function sliceWindow(numbered, start, end, maxChars) {
    let out = "";
    ...
    if (first)
        return { text: "", actualStart: start, actualEnd: start, truncated };
    return { text: out, actualStart, actualEnd, truncated };
}
async function readFileCapped(abs, display, signal) {
    const bytes = await readFile(abs, { signal }).catch(() => {
        signal?.throwIfAborted();
        throw new Error(`cannot read ${display}`);
    });
    if (bytes.length > MAX_INDEX_FILE_BYTES)
        throw new Error(`${display} exceeds max ${MAX_INDEX_FILE_BYTES} bytes`);
    signal?.throwIfAborted();
    try {
...
export async function readWindowsFallback(configuredCwd, args, signal) {
    signal?.throwIfAborted();
    ...
    if (refs.length === 0)
        throw new Error("read requires path, ref, or refs");
    if (refs.length > MAX_READ_REFS)
        throw new Error(`read exceeds max ${MAX_READ_REFS} windows`);
    const windows = [];
    for (const spec of refs) {
        signal?.throwIfAborted();
        const { abs, display } = await jailPath(root, spec.path);
        const text = await readFileCapped(abs, display, signal);
        const numbered = rustLines(text).map((line, index) => [index + 1, line]);
...
function collectEdits(args) {
    if (Array.isArray(args.edits))
        return args.edits.map(parseEditValue);
    if (typeof args.path === "string")
        return [parseEditValue(args)];
    return [];
}
...
export async function editFilesFallback(configuredCwd, args, signal) {
    ...
    try {
        ...
        if (editTails.get(root) === tail)
            editTails.delete(root);
    }
}
async function applyEdits(root, args, signal) {
    signal?.throwIfAborted();
    const edits = collectEdits(args);
    if (edits.length === 0)
        throw new Error("edit requires path+oldText+newText or edits[]");
    if (edits.length > MAX_EDITS)
        throw new Error(`edit exceeds max ${MAX_EDITS} replacements`);
    ...
    for (const edit of edits) {
        ...
        if (first === -1)
            throw new Error("oldText must match exactly once (found 0)");
        ...
        applied.push({
            ...
            added: added.lines,
            ...(removed.more || added.more ? { truncated: true } : {}),
        });
    }
    // Phase 2: every edit validated — write each touched file once.
    for (const entry of plan.values()) {
        if (entry.rewritten === entry.original)
            continue;
        signal?.throwIfAborted();
