...
const METHOD_ALIASES: Record<string, CodemodeHostMethod> = {
...
function mergeRest(base: Record<string, unknown>, rest: unknown): Record<string, unknown> {
  if (!isPlainObject(rest)) return base;
  return { ...rest, ...base };
}
...
function suggestMethod(name: string): CodemodeHostMethod | undefined {
  const needle = name.toLowerCase();
  if (!needle) return undefined;
  const maxDistance = Math.max(1, Math.floor(needle.length / 3));
  let best: { candidate: CodemodeHostMethod; distance: number } | undefined;
...
export function resolveHostMethod(method: string): CodemodeHostMethod | undefined {
  if ((CODEMODE_HOST_METHODS as readonly string[]).includes(method)) {
    return method as CodemodeHostMethod;
  }
  const aliased = METHOD_ALIASES[method] ?? METHOD_ALIASES[method.toLowerCase()];
  if (aliased) return aliased;
  const camel = method.replace(/_([a-z])/gu, (_all, letter: string) => letter.toUpperCase());
  if (camel !== method && (CODEMODE_HOST_METHODS as readonly string[]).includes(camel)) {
    return camel as CodemodeHostMethod;
  }
  return undefined;
}

/** Turn positional guest calls into the host object shape. */
export function packGuestCall(method: string, args: unknown[]): Record<string, unknown> {
  if (args.length === 0) return {};
  const first = args[0];
  const rest = args[1];
  if (typeof first === "string") {
    if (SYMBOL_METHODS.has(method)) return mergeRest({ symbol: first }, rest);
    if (method === "imports") return mergeRest({ module: first }, rest);
    if (method === "read") {
      if (typeof args[1] === "number") {
        const packed: Record<string, unknown> = { path: first, start: args[1] };
        if (typeof args[2] === "number") packed.end = args[2];
        return packed;
      }
    ...
    if (method === "catalogDescribe") return mergeRest({ name: first }, rest);
  ...
  throw new Error(
    `asgrep.${method}: pass a string or object (got ${typeof first}). Example: asgrep.search("auth") or asgrep.search({ query: "auth" })`,
  );
}

/** Accept query as a symbol alias and fold in:/fileFilter into the query string. */
export function coerceHostArgs(method: string, input: Record<string, unknown>): Record<string, unknown> {
  const args = { ...input };
  if (SYMBOL_METHODS.has(method) && typeof args.symbol !== "string") {
    const alias = args.query;
    if (typeof alias === "string" && alias.trim()) args.symbol = alias.trim();
  }
  if (QUERY_METHODS.has(method)) {
    const query = typeof args.query === "string" ? args.query : "";
    const scoped = applyQueryScope(query, args);
    if (scoped) args.query = scoped;
  }
  return args;
}

export function applyQueryScope(query: string, args: Record<string, unknown>): string | undefined {
  const scope = scopeToken(args.in) ?? scopeToken(args.fileFilter) ?? scopeToken(args.file_filter);
  if (!scope) return query.trim() ? query : undefined;
  if (query.split(/\s+/u).some((token) => token.startsWith("in:"))) return query;
  const trimmed = query.trim();
  return trimmed ? `in:${scope} ${trimmed}` : `in:${scope}`;
}

export function unknownMethodError(method: string): string {
...
export function timeoutHint(message: string): string {
  if (!/timeout after \d+ms|timed out after \d+ms/i.test(message)) return message;
...
function looksLikeBareExpression(code: string): boolean {
  ...
  if (/^(?:const|let|var|function|class|if|for|while|switch|try|async|import|export|throw|do|break|continue|debugger|yield|with|else|case|default)\b/u.test(trimmed)) return false;
