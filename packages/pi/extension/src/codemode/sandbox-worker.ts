...
type Limits = {
  bridgeCalls: number;
  bridgeRequestChars: number;
  errorChars: number;
  logLines: number;
  logChars: number;
  logLineChars: number;
  resultJsonChars: number;
  serializeTimeoutMs: number;
};

type SandboxWorkerData = {
  code: string;
  timeoutMs: number;
  limits: Limits;
};
...
const port = (() => {
  if (!parentPort) throw new Error("codemode sandbox requires a parent port");
  return parentPort;
})();

const data = workerData as SandboxWorkerData;
const pending = new Map<number, (payload: string) => void>();
const outgoing: Array<{ id: number; method: string; payload: string }> = [];
let nextCallId = 0;
let flushScheduled = false;

port.on("message", (message: CallResult) => {
  if (message.type !== "callResult") return;
...
const bridge = (method: string, payload: string): Promise<string> =>
  new Promise((resolve) => {
    ...
    if (!flushScheduled) {
      ...
      queueMicrotask(() => {
        ...
        if (calls.length > 0) port.postMessage({ type: "calls", calls });
      });
    }
  });
...
async function run(): Promise<void> {
  const logs: string[] = [];
  let logChars = 0;
  const logBridge = (line: string): void => {
    if (logs.length >= data.limits.logLines || logChars >= data.limits.logChars) return;
  ...
  try {
    ...
    const setResult = context.__asgrepSetResult as ((value: unknown) => void) | undefined;
...
function bootstrap(limits: Limits): string {
  return `
  {
