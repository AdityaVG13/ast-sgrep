...
export type BatchCapableHost = ConnectorHost & {
  ...
  ): Promise<{ results: Array<{ id: string; ok: boolean; value?: unknown; error?: string }>; mode?: string; wall_ms?: number }>;
};
...
export function createCodemodeDispatcher(host: BatchCapableHost): {
  ...
  const dispatchHost: ConnectorHost = {
    run(args, context, options) {
      return new Promise<MachineEnvelope>((resolve, reject) => {
        const item: Pending = { args, context, resolve, reject };
...
function toolFromArgs(args: readonly string[]): string {
  ...
  if (query.startsWith("defs:")) return "defs";
  if (query.startsWith("callers:")) return "callers";
  if (query.startsWith("imports:")) return "imports";
  return "search";
}
...
function asEnvelope(value: unknown): MachineEnvelope {
  if (value && typeof value === "object" && (value as MachineEnvelope).tool === "asgrep") {
...
export async function runNativeBatch(
  ...
  try {
    ...
    const envelope = await run(
      ["codemode-batch", "--requests", requestsPath, "--json"],
      context,
      options,
    );
    const results = Array.isArray(envelope.results) ? envelope.results as Array<{ id: string; ok: boolean; value?: unknown; error?: string }> : [];
    const out: { results: Array<{ id: string; ok: boolean; value?: unknown; error?: string }>; mode?: string; wall_ms?: number } = { results };
