...
export function incompatibleStatusFailure(cause: unknown): boolean {
...
export function indexCompletion(response: MachineEnvelope, requireWalkErrors: boolean): { failed: number; walkErrors: boolean } {
...
export function indexPathFor(root: string, env: NodeJS.ProcessEnv): string {
...
export function indexQuarantines(indexPath: string): string[] {
...
export function throwIndexRebuildFailed(cause: unknown, indexPath: string, quarantinesBefore: ReadonlySet<string>): never {
...
export function inspectIndexFile(path: string): "missing" | "incompatible" | number {
  ...
  try {
    ...
    if (!Number.isSafeInteger(version) || version <= 0) return "incompatible";
    return version;
  } catch {
    return "incompatible";
  } finally {
