...
export function bounded(text: string, maxChars = MAX_CONTENT_CHARS): string {
  return boundedText(text, maxChars);
}

const noteText = (notes: string[] = []): string => bounded(notes.map(note => `  ! ${note}`).join("\n"), 2000);
 ...
 */
const OPERATIONAL_FAILURES: ReadonlyArray<{ pattern: RegExp; hint?: string }> = [
  {
...
export function errorDetails(cause: unknown, signal?: AbortSignal): { code: string; message: string; details: Readonly<Record<string, unknown>> } {
  ...
  for (const family of OPERATIONAL_FAILURES) {
    if (!family.pattern.test(message)) continue;
    return {
