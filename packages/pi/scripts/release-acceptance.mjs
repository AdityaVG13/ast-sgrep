...
const expectReject = (label, callback, rejected) => {
  try { callback(); } catch (error) { rejected.push(`${label}=${error.message.split(':')[0]}`); return; }
...
const requiredFilesFor = (state, artifact) => artifact.layer === 'native'
...
const validateFiles = (state, artifact) => {
  ...
  for (const entry of requiredFilesFor(state, artifact)) if (!files.includes(entry)) fail('ASGREP_RELEASE_CONTENT_MISSING', `${artifact.name} is missing ${entry}`);
...
if (!selected) fail('ASGREP_RELEASE_USAGE', 'pack | verify | gate | publish | fixture-native | self-test');
await selected();
