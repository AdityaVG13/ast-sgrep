...
const piCli = path.join(root, 'packages/pi/extension', 'node_modules', '@earendil-works', 'pi-coding-agent', 'dist', 'cli.js');
...
async function stageArtifacts(version, binary) {
  ...
  await replace(path.join(launcher, 'src', 'index.js'), 'const VERSION = "1.4.0";', `const VERSION = "${version}";`);
