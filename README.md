...
**v1.3.1** · 8 languages · lexical + AST graph + **semantic symbol search** (on by default, no API key)
...
For the standalone CLI, after `1.3.1` is published to crates.io, install with:

```bash
cargo install ast-sgrep-cli --version 1.3.1 --locked
```
...
**v1.3.1.** Hybrid search, semantic layer, LSP, MCP, agent JSON, and IVF path are in place.
