...
use crate::provider::EmbedBackendKind;
...
impl Default for HashedEmbedder {
    fn default() -> Self {
        Self { inner: SemanticLocalEmbedding, model_id: format!("hashed-{SEMANTIC_DIM}") }
    }
}
...
impl Embedder for OllamaEmbedder {
    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        texts.iter().map(|t| embed_via_ollama(t, &self.config).map_err(|e| anyhow!(e))).collect()
    }
    fn dim(&self) -> usize {
        0
    }
...
impl Embedder for CloudEmbedder {
    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        texts.iter().map(|t| embed_via_api(t, &self.config).map_err(|e| anyhow!(e))).collect()
    }
    fn dim(&self) -> usize {
        0
    }
...
pub fn embedder_for(kind: EmbedBackendKind) -> Option<Box<dyn Embedder>> {
    match kind {
        EmbedBackendKind::Cloud => {
            CloudEmbeddingConfig::from_env().map(|c| Box::new(CloudEmbedder::new(c)) as Box<dyn Embedder>)
        }
        EmbedBackendKind::Ollama => {
            OllamaEmbeddingConfig::from_env().map(|c| Box::new(OllamaEmbedder::new(c)) as Box<dyn Embedder>)
        }
