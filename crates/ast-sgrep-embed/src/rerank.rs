...
fn load(config: &RerankConfig) -> Result<SharedReranker> {
    let mut cache = RERANKERS
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    if let Some(entry) = cache.get(config) {
        return entry.clone().map_err(anyhow::Error::msg);
    }
...
pub fn rerank(query: &str, documents: &[String]) -> Result<Vec<RerankScore>> {
    ...
    let mut model = model
        .lock()
        .map_err(|_| anyhow!("local reranker mutex poisoned"))?;
    ...
    for (batch_index, batch) in documents.chunks(batch_size).enumerate() {
        ...
        let results = model
            .rerank(query, &refs, false, Some(batch.len()))
