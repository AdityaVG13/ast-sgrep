...
impl RerankConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            model: resolve_reranker_model(std::env::var("ASGREP_RERANK_MODEL").ok().as_deref())?,
            cache_dir: std::env::var_os("ASGREP_RERANK_CACHE_DIR")
                .filter(|v| !v.is_empty())
                .map(PathBuf::from)
                .unwrap_or_else(crate::neural_default_cache_dir),
            max_length: env_usize("ASGREP_RERANK_MAX_LENGTH", 128),
...
fn resolve_reranker_model(value: Option<&str>) -> Result<RerankerModel> {
    match value.map(|v| v.trim()) {
        None | Some("") => Ok(RerankerModel::JINARerankerV1TurboEn),
        Some(trimmed) => match trimmed.to_ascii_lowercase().as_str() {
            "jina-v1-turbo-en" => Ok(RerankerModel::JINARerankerV1TurboEn),
            "bge-reranker-base" => Ok(RerankerModel::BGERerankerBase),
            "bge-reranker-v2-m3" => Ok(RerankerModel::BGERerankerV2M3),
            "jina-v2-base-multilingual" => Ok(RerankerModel::JINARerankerV2BaseMultiligual),
            _ => Err(anyhow!(
                "unknown reranker model {:?}. Accepted aliases: \
                 bge-reranker-base, bge-reranker-v2-m3, jina-v1-turbo-en, \
                 jina-v2-base-multilingual (CC-BY-NC-4.0, non-commercial)",
                trimmed
            )),
        },
    }
...
static RERANKERS: LazyLock<Mutex<HashMap<RerankConfig, CacheEntry>>> =
...
fn load(config: &RerankConfig) -> Result<SharedReranker> {
    ...
    cache.insert(config.clone(), loaded.clone());
    loaded.map_err(anyhow::Error::msg)
}
pub fn rerank(query: &str, documents: &[String]) -> Result<Vec<RerankScore>> {
    if documents.is_empty() { return Ok(vec![]); }
    let model = load(&RerankConfig::from_env()?)?;
    let batch_size = env_usize("ASGREP_RERANK_BATCH_SIZE", 1).min(documents.len());
    let mut model = model.lock().map_err(|_| anyhow!("local reranker mutex poisoned"))?;
    let mut scores = Vec::with_capacity(documents.len());
    for (batch_index, batch) in documents.chunks(batch_size).enumerate() {
        let document_refs = batch.iter().map(String::as_str).collect::<Vec<_>>();
        let results = model
            .rerank(query, &document_refs, false, Some(batch.len()))
            .map_err(|error| anyhow!("local rerank failed: {error}"))?;
        let offset = batch_index * batch_size;
        scores.extend(results.into_iter().map(|r| RerankScore {
