...
fn base<'a>(path: &'a str, lines: &'a [(u32, String)], hash: &'a str) -> UpsertFileInput<'a> {
    UpsertFileInput {
        ...
        semantic_chunks: &[], embed_semantic: false, embed_backend: ast_sgrep_embed::EmbedPreference::Auto,
    }
...
#[test] fn semantic_mutation_removes_ivf_before_it_can_be_reloaded() {
    ...
    std::fs::write(&sidecar, b"stale sidecar").unwrap(); let lines = [(1, "semantic content".into())]; let chunks = [ast_sgrep_core::semantic_chunk::SemanticChunkInput {
        symbol_name: "example".into(), kind: "function".into(), line_start: 1, line_end: 1, excerpt: "semantic content".into(), callers: vec![], callees: vec![], doc: String::new(), scope: String::new(),
...
#[test] fn re_upsert_does_not_leave_stale_fts_rows() {
    ...
    assert_eq!(count_match(&store, "lines_fts", "alpha"), 1); assert_eq!(count_match(&store, "lines_trigram", "alp"), 1);
...
#[test] fn remove_file_clears_all_per_file_tables() {
    ...
    let callers = [CallerRow { caller: "foo".into(), callee: "bar".into(), line_no: 1, byte_start: 0, byte_end: 3 }]; let imports = [ImportRow { module_path: "os".into(), line_no: 1 }];
    let pattern_nodes = [PatternNode { signature: "sig".into(), line_start: 1, line_end: 1, excerpt: "ex".into() }]; let lines = [(1, "import os".into()), (2, "foo(bar)".into())];
    let mut input = base(path, &lines, "hash"); input.symbols = &symbols; input.callers = &callers; input.imports = &imports; input.pattern_nodes = &pattern_nodes; let file_id = store.upsert_file(input).unwrap();
    store.connection().execute("INSERT INTO embeddings (file_id, line_no, vector) VALUES (?1, ?2, ?3)", rusqlite::params![file_id, 1u32, vec![0u8; 8]]).unwrap(); store.connection().execute(
    ...
    ).unwrap(); store.remove_file(path).unwrap(); for table in ["lines", "lines_fts", "lines_trigram", "symbols", "callers", "imports", "pattern_nodes", "embeddings", "semantic_chunks"] {
...
#[test] fn re_upsert_many_files_is_linear() {
    ...
    let lines = [(1, "hello world".into()), (2, "foo bar baz".into())]; let lines2 = [(1, "goodbye world".into()), (2, "qux corge grault".into())]; let run = |lines: &[(u32, String)], prefix: &str, offset: usize| {
        store.begin_bulk_tx().unwrap(); let t0 = std::time::Instant::now(); for (i, path) in paths.iter().enumerate() {
            let hash = format!("{prefix}{i}"); let mut input = base(path, lines, &hash); input.mtime_secs = (i + offset) as i64; store.upsert_file(input).unwrap();
        } store.commit_bulk_tx().unwrap(); t0.elapsed()
...
#[test] fn same_span_body_edit_refreshes_semantic_chunks() {
    ...
    let callers: [CallerRow; 0] = []; let imports: [ImportRow; 0] = []; let lines_v1 = [(1, "def compute():".into()), (2, "    return ALPHA_TOKEN_111".into()), (3, "".into())];
    let lines_v2 = [(1, "def compute():".into()), (2, "    return BETA_TOKEN_222".into()), (3, "".into())];
    let chunks_v1 = build_semantic_chunks(&symbols, &callers, &lines_v1); let chunks_v2 = build_semantic_chunks(&symbols, &callers, &lines_v2);
    assert!(!chunks_v1.is_empty() && !chunks_v2.is_empty()); assert_ne!(chunks_v1[0].excerpt, chunks_v2[0].excerpt);
    let pat_v1 = [PatternNode { signature: "fn compute".into(), line_start: 1, line_end: 3, excerpt: "return ALPHA_TOKEN_111".into() }];
    let pat_v2 = [PatternNode { signature: "fn compute".into(), line_start: 1, line_end: 3, excerpt: "return BETA_TOKEN_222".into() }];
    let upsert = |lines: &[(u32, String)], chunks: &[ast_sgrep_core::semantic_chunk::SemanticChunkInput], pats: &[PatternNode], hash: &str| {
        let mut input = base(path, lines, hash); input.symbols = &symbols; input.callers = &callers; input.imports = &imports; input.pattern_nodes = pats; input.semantic_chunks = chunks;
        input.embed_semantic = true; input.embed_backend = ast_sgrep_embed::EmbedPreference::Semantic; store.upsert_file(input).unwrap();
    }; upsert(&lines_v1, &chunks_v1, &pat_v1, "hash_alpha"); let rows_v1 = store.all_semantic_chunks(None).unwrap(); assert_eq!(rows_v1.len(), 1);
    let (text_v1, vec_v1) = (rows_v1[0].4.clone(), rows_v1[0].5.clone()); assert!(text_v1.contains("ALPHA_TOKEN_111"));
    upsert(&lines_v2, &chunks_v2, &pat_v2, "hash_beta"); let rows_v2 = store.all_semantic_chunks(None).unwrap(); assert_eq!(rows_v2.len(), 1);
    assert!(rows_v2[0].4.contains("BETA_TOKEN_222")); assert!(!rows_v2[0].4.contains("ALPHA_TOKEN_111")); assert_ne!(text_v1, rows_v2[0].4); assert_ne!(vec_v1, rows_v2[0].5);
    let excerpt: String = store.connection().query_row("SELECT excerpt FROM pattern_nodes LIMIT 1", [], |r| r.get(0)).unwrap(); assert!(excerpt.contains("BETA_TOKEN_222"), "pattern excerpt must refresh: {excerpt}");
}
