mod embed_support;
pub(crate) mod sql; mod sqlite; pub use sql::integrity_check; pub use sqlite:: { CallerRow, ImportRow, IndexStore, IndexedLineRow, SymbolLocationRow, SymbolRow, UpsertFileInput, }; use std::path::{Path, PathBuf}; pub const INDEX_DIR: &str = ".asgrep"; pub const INDEX_DB: &str = "index.db";
fn as_db_path(path: PathBuf) -> PathBuf { if path.extension().is_some_and(|e| e == "db") { path } else { path.join(INDEX_DB) } } pub fn index_db_path(root: &Path, index_path: Option<&Path>) -> PathBuf {
