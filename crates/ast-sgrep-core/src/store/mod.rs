...
pub(crate) mod sql;
mod sqlite;
pub use pragmas::integrity_check;
pub use sqlite::{
...
pub fn index_db_path(root: &Path, index_path: Option<&Path>) -> PathBuf {
    if let Some(path) = index_path { return as_db_path(path.to_path_buf()); }
    if let Ok(env_path) = std::env::var("ASGREP_INDEX_PATH") { return as_db_path(PathBuf::from(env_path)); }
    let local = root.join(INDEX_DIR).join(INDEX_DB);
    if local.exists() { return local; }
    if std::env::var("ASGREP_USE_CACHE").ok().as_deref() == Some("1") { return cache_index_path(root); }
    local
}
