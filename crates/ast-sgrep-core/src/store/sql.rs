...
pub fn optional_row<T, F>(conn: &Connection, sql: &str, bind: &[&dyn ToSql], map: F) -> Result<Option<T>>
where F: FnOnce(&rusqlite::Row<'_>) -> rusqlite::Result<T>,
{
...
pub fn calls_matching(conn: &Connection, column: &str, name: &str) -> Result<Vec<(String, u32, String, String)>> {
    ...
    let mut stmt = conn.prepare_cached(&sql)?;
    let rows = stmt.query_map(params![name], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)))?;
    rows.collect::<std::result::Result<Vec<_>, _>>().map_err(Into::into)
}

pub fn append_lang_filter(parts: &mut Vec<String>, bind: &mut Vec<String>, lang: Option<&str>) {
    if let Some(lang) = lang { parts.push("f.language = ?".into()); bind.push(lang.into()); }
}
...
fn escape_like_term(term: &str) -> String {
    ...
    for ch in term.chars() {
        match ch {
            '\\' => out.push_str("\\\\"), '%' => out.push_str("\\%"), '_' => out.push_str("\\_"),
...
pub fn caller_terms_filter(terms: &[String], lang_filter: Option<&str>) -> (String, Vec<String>) {
    ...
    if !terms.is_empty() {
        ...
        for t in &escaped { bind.push(t.clone()); bind.push(t.clone()); }
        parts.push(format!("({})", conds.join(" OR ")));
    }
    append_lang_filter(&mut parts, &mut bind, lang_filter);
    (where_clause(&parts), bind)
...
{
    let mut out = Vec::new();
    while let Some(row) = rows.next()? { out.push(map(row)?); }
    Ok(out)
}

pub fn query_map_rows<T, F>(conn: &Connection, sql: &str, lang: Option<&str>, map: F) -> Result<Vec<T>>
where F: FnMut(&rusqlite::Row<'_>) -> rusqlite::Result<T>,
{
    let mut stmt = conn.prepare_cached(sql)?;
    if let Some(lang) = lang { collect_mapped_rows(stmt.query(params![lang])?, map) }
...
pub fn delete_file_children(conn: &Connection, file_id: i64) -> Result<()> {
    conn.prepare_cached("DELETE FROM lines_fts WHERE file_id = ?1")?.execute(params![file_id])?;
    conn.prepare_cached(
