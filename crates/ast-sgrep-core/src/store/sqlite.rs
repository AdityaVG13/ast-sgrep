...
impl IndexStore {
    ...
    fn evict_embed_cache(&self, max_entries: usize) -> Result<()> {
        if max_entries == 0 {
            self.conn.execute("DELETE FROM embed_cache", [])?;
            return Ok(());
        }
        let count: i64 = self
            .conn
    ...
    fn embed_parallel(
        ...
        for (i, text) in texts.into_iter().enumerate() {
            ...
            let r = miss_it.next().ok_or_else(|| {
                crate::StoreError::Other("embedding result length mismatch".into())
            })?;
