...
use super::sql::{
    ...
    query_cached_map, query_limit_map, query_map_rows, read_legacy_emb, read_sem_row, where_clause,
    SCHEMA_DDL,
};
...
pub struct PatternNodeRow {
    pub path: String, pub language: Option<String>, pub line_start: u32, pub line_end: u32, pub excerpt: String,
}
...
pub struct CallerRow {
    pub caller: String, pub callee: String, pub line_no: u32, pub byte_start: usize, pub byte_end: usize,
}
#[derive(Debug, Clone)]
pub struct ImportRow { pub module_path: String, pub line_no: u32 }
#[derive(Debug, Clone)]
pub struct SymbolLocationRow {
    pub path: String, pub name: String, pub language: Option<String>, pub line_start: u32, pub line_end: u32,
}
...
impl IndexStore {
    pub fn open(root: &Path, index_path: Option<&Path>) -> Result<Self> {
        ...
        store.init_schema()?; init_cache_seq(&store.conn, &store.cache_seq)?; Ok(store)
    }
    fn init_schema(&self) -> Result<()> {
        ...
        if version < 3 {
            self.conn.execute_batch("INSERT INTO lines_trigram(rowid, content) SELECT rowid, content FROM lines;")?;
        }
        self.conn.execute_batch(&format!("PRAGMA user_version = {SCHEMA_VERSION}"))?; Ok(())
    }
    ...
    pub fn delete_meta(&self, key: &str) -> Result<()> {
        self.conn.execute("DELETE FROM meta WHERE key = ?1", params![key])?; Ok(())
    }
    pub fn file_id(&self, rel_path: &str) -> Result<Option<i64>> {
        optional_row(&self.conn, "SELECT id FROM files WHERE path = ?1", &[&rel_path], |r| r.get(0))
    }
    pub fn begin_file_tx(&self) -> Result<()> {
        if !self.conn.is_autocommit() {
            return Ok(());
        }
        // OFF for the tx (WAL still crash-safe for committed frames); avoid re-NORMAL
        // after every file — that fsync dominated sub-ms single-file updates.
        self.conn
    ...
    pub fn rollback_bulk_tx(&self) -> Result<()> {
        if self.conn.is_autocommit() { return Ok(()); }
        self.file_tx_active.set(false); let _ = self.conn.execute_batch("ROLLBACK"); Ok(())
    }
    pub fn clear_all_data(&self) -> Result<()> {
        self.conn.execute_batch(
            "DELETE FROM lines_trigram; DELETE FROM lines_fts; DELETE FROM semantic_chunks; DELETE FROM pattern_nodes;             DELETE FROM embeddings; DELETE FROM imports; DELETE FROM callers; DELETE FROM symbols; DELETE FROM lines; DELETE FROM files;",
        )?;
    ...
    pub fn upsert_file(&self, input: UpsertFileInput<'_>) -> Result<i64> {
        ...
        let struct_fp = structure_fingerprint(
            input.symbols, input.callers, input.imports, input.pattern_nodes, input.semantic_chunks,
        );
        ...
        if let Some(file_id) = self.file_id(input.rel_path)? {
            if self.get_meta(&struct_key)?.as_deref() == Some(struct_fp.as_str()) {
                ...
                return match self.refresh_lines_only(
                    file_id, input.language, input.mtime_secs, input.mtime_nanos,
                    input.content_hash, input.lines, input.eol, input.rel_path,
                ) {
                    Ok(id) => { self.commit_file_tx()?; Ok(id) }
        ...
        let (cache_hits, cache_misses) = (emb.cache_hits.len(), emb.chunks.len().saturating_sub(emb.cache_hits.len()));
        self.begin_file_tx()?;
        match self.upsert_file_inner(input, &emb.chunks, &struct_key, &struct_fp) {
            Ok(id) => {
                if !emb.cache_entries.is_empty() {
                    if let Err(e) = insert_embed_cache_entries(&self.conn, &self.cache_seq, &emb.cache_entries) {
                    ...
                    let hits: Vec<_> = emb.cache_hits.iter()
                        .map(|h| (h.chunk_hash.clone(), h.model_id.clone())).collect();
                ...
                for (key, delta) in [("embed_cache_hits", cache_hits), ("embed_cache_misses", cache_misses)] {
                    if delta == 0 { continue; }
                    let total = self.get_meta(key)?.and_then(|v| v.parse::<u64>().ok()).unwrap_or(0)
    ...
    /// Refresh lines/FTS only when structure fingerprint is unchanged (batch delete).
    pub fn refresh_lines_only(
        &self, file_id: i64, lang: Option<&str>, mtime_secs: i64, mtime_nanos: u32, hash: &str,
        lines: &[(u32, String)], eol: &str, rel_path: &str,
    ) -> Result<i64> {
        // Prefer append/truncate of trailing lines when the common prefix is unchanged
        // (typical trailing-comment / trivia edits from watch / sub1ms gate).
        ...
        if common == existing.len() && lines.len() >= existing.len() {
            // Pure append of trailing lines.
            let extra = &lines[common..];
            if !extra.is_empty() {
                self.insert_lines_no_trigram(file_id, extra)?;
            }
        } else if common == lines.len() && existing.len() > lines.len() {
            // Pure truncate of trailing lines.
            ...
            // Full line rewrite must clear FTS + trigram so search never sees stale tokens.
    ...
    fn upsert_file_inner(
        &self, input: UpsertFileInput<'_>, emb: &[EmbeddedChunk], struct_key: &str, struct_fp: &str,
    ) -> Result<i64> {
        ...
        crate::semantic_ann::mark_semantic_ivf_stale(self); Ok(file_id)
    }
    fn upsert_file_row(
    ...
    fn persist_embed_metadata(
        ...
        if let Some(k) = kind {
            ...
            } else { self.delete_meta("embed_model")?; }
        }
    ...
    pub fn status(&self) -> Result<IndexStatus> {
        let (fc, lc, sc, cc, ic, sec): (i64, i64, i64, i64, i64, i64) = self.conn.query_row(
            "SELECT (SELECT COUNT(*) FROM files),(SELECT COUNT(*) FROM lines),(SELECT COUNT(*) FROM symbols),                    (SELECT COUNT(*) FROM callers),(SELECT COUNT(*) FROM imports),(SELECT COUNT(*) FROM semantic_chunks)",
            [], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?, r.get(5)?)),
        )?;
        Ok(IndexStatus {
            root: self.root.display().to_string(), index_path: self.db_path.display().to_string(),
    ...
    /// True when indexed lines ≥ threshold (LIMIT probe; avoids full COUNT on large indexes).
    ...
    pub fn semantic_chunk_max_id(&self) -> Result<Option<i64>> {
        optional_row(&self.conn, "SELECT MAX(id) FROM semantic_chunks", &[], |r| r.get::<_, Option<i64>>(0)).map(Option::flatten)
    }
    ...
    pub fn semantic_chunk_ids(&self, lang: Option<&str>) -> Result<Vec<i64>> {
        ...
        } else { ("SELECT id FROM semantic_chunks ORDER BY id", None) };
    ...
    pub fn semantic_chunks_by_ids(&self, ids: &[i64]) -> Result<Vec<(i64, ast_sgrep_embed::SemanticChunkRow)>> {
        ...
        for batch in ids.chunks(500) {
            ...
            let sql = format!("SELECT sc.id, f.path, sc.line_start, sc.line_end, sc.symbol_name, sc.text, sc.vector FROM semantic_chunks sc JOIN files f ON f.id=sc.file_id WHERE sc.id IN ({ph})");
            let mut stmt = self.conn.prepare(&sql)?;
            let rows = stmt.query_map(rusqlite::params_from_iter(batch.iter()), |r| {
                let id = r.get(0)?; let v: Vec<u8> = r.get(6)?;
                Ok((id, (r.get(1)?, r.get(2)?, r.get(3)?, r.get::<_, Option<String>>(4)?.unwrap_or_default(), r.get(5)?,
    ...
    pub fn symbol_at_line(&self, path: &str, line: u32) -> Result<Option<SymbolLocationRow>> {
        optional_row(
            &self.conn,
            &format!("{SYM_LOC} WHERE f.path=?1 AND s.line_start<=?2 AND s.line_end>=?2 ORDER BY (s.line_end-s.line_start), s.line_start DESC, s.name LIMIT 1"),
            &[&path, &line], read_sym_loc,
        )
    }
    ...
    pub fn symbols_named(&self, name: &str, limit: usize) -> Result<Vec<SymbolLocationRow>> {
        query_cached_map(
            &self.conn,
            &format!("{SYM_LOC} WHERE s.name=?1 ORDER BY f.path, s.line_start, s.line_end LIMIT ?2"),
            params![name, limit as i64], read_sym_loc,
        )
    }
    pub fn imports_from_file(&self, path: &str) -> Result<Vec<ImportRow>> {
        query_cached_map(
            ...
            params![path], |r| Ok(ImportRow { module_path: r.get(0)?, line_no: r.get(1)? }),
        )
    }
    pub fn resolve_module_path(&self, from_file: &str, module: &str) -> Result<Vec<String>> {
        ...
        if let Some(rest) = slash.strip_prefix("crate/") {
            ...
            let mut base = parent.to_path_buf(); let mut rest = slash.as_str();
        ...
        let mut out = Vec::new();
        for c in cands { if self.file_exists(&c)? { out.push(c); } }
        Ok(out)
    }
    ...
    pub fn symbols_matching(&self, name: &str, limit: usize) -> Result<Vec<SymbolFileRow>> {
        let sql = format!(
            "{SYM_FILE} WHERE lower(s.name) LIKE '%' || lower(?1) || '%' ESCAPE '\\' ORDER BY f.path, s.line_start LIMIT ?2"
        );
    ...
    pub fn files_importing_module(&self, module: &str, limit: usize) -> Result<Vec<ImportQueryRow>> {
        let sql = format!(
            "{IMPORT_SELECT} WHERE lower(i.module_path) LIKE '%' || lower(?1) || '%' ESCAPE '\\' ORDER BY f.path, i.line_no LIMIT ?2"
        );
    ...
    pub fn query_imports(&self, module: Option<&str>, lang: Option<&str>, limit: usize) -> Result<Vec<ImportQueryRow>> {
        ...
        if module.is_none_or(|m| m.is_empty()) {
            let mut parts = Vec::new(); let mut bind = Vec::new();
