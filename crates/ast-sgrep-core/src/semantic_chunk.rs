use ast_sgrep_embed::expand_concepts; use crate::store::{CallerRow, SymbolRow}; #[derive(Debug, Clone)] pub struct SemanticChunkInput {
    pub symbol_name: String, pub kind: String, pub line_start: u32, pub line_end: u32, pub excerpt: String, pub callers: Vec<String>, pub callees: Vec<String>, pub doc: String, pub scope: String,
} pub fn build_semantic_chunks(
    symbols: &[SymbolRow], callers: &[CallerRow], lines: &[(u32, String)],
    ...
    let mut raw = format!("symbol: {} kind: {}", chunk.symbol_name, chunk.kind); if !chunk.scope.is_empty() {
    ...
    let mut collected = Vec::new(); let mut expect = line_start.saturating_sub(1); for (no, content) in lines.iter().rev() {
        if *no > expect || expect == 0 { continue; } if *no < expect || collected.len() >= DOC_LOOKBACK_LINES { break; } let Some(text) = strip_comment_marker(content) else {
            break;
        }; collected.push(text); expect -= 1;
