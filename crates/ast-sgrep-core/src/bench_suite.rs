use std::path::PathBuf; #[derive(Debug, Clone, Copy)] pub struct BenchCase {
    pub name: &'static str, pub query: &'static str, pub min_hits: usize,
} #[derive(Debug, Clone)] pub struct BenchFixture {
    pub name: &'static str, pub root: PathBuf, pub suite: &'static str,
    ...
    match name {
        "default" => Some(DEFAULT_SUITE), "self" => Some(SELF_SUITE), _ => None,
    }
...
pub fn list_fixture_names() -> Vec<&'static str> { bench_fixtures().iter().map(|f| f.name).collect() } #[derive(Debug, Clone, Copy, PartialEq)] pub struct RankingStability {
    pub jaccard: f64, pub rank_correlation: f64,
    ...
    let rank_correlation = if shared.len() < 2 {
        ...
        let n = shared.len() as f64; let sq: f64 = shared.iter().map(|(a, b)| {
            let d = *a as f64 - *b as f64; d * d
        }).sum(); 1.0 - (6.0 * sq) / (n * (n * n - 1.0))
