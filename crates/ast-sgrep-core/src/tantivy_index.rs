...
impl TantivySidecar {
    ...
    pub fn open_existing_for_search(root: &Path, index_path: Option<&Path>) -> Result<Option<Self>> {
        ...
        // Never treat a zero-length / freshly-touched empty file as ready (y1oy.4).
        if metadata.len() == 0 {
            return Ok(None);
        }
        let conn = Connection::open(&db_path)?;
    ...
    /// True when the sidecar has indexed at least one line (y1oy.4).
    pub fn is_search_ready(&self) -> Result<bool> {
        let lines: Option<String> = self
            .conn
            .query_row(
                "SELECT value FROM meta WHERE key = 'lines'",
                [],
                |r| r.get(0),
            )
            .ok();
