...
pub struct IgnoreMatcher {
    root: PathBuf,
    chains: RefCell<HashMap<String, Rc<Vec<Rule>>>>,
}
impl IgnoreMatcher {
    ...
    pub fn is_ignored(&self, rel: &Path) -> bool {
        ...
        if parent_dir_excluded(&rel_str, &rules) { return true; }
    ...
    fn chain_for(&self, prefix: &str) -> Rc<Vec<Rule>> {
        if let Some(hit) = self.chains.borrow().get(prefix) { return Rc::clone(hit); }
...
fn parse_rule(base: &str, line: &str) -> Rule {
    ...
    let pat = if negate { line[1..].trim() } else { line.trim() };
    Rule {
        base: base.to_string(),
...
fn rel_under_base<'a>(rule: &Rule, rel_str: &'a str) -> Option<&'a str> {
    if rule.base.is_empty() { return Some(rel_str); }
    rel_str
...
fn matches_file(rule: &Rule, rel_str: &str) -> bool {
    !rule.dir_only
        && rel_under_base(rule, rel_str).is_some_and(|path| pattern_matches(&rule.pattern, path, false))
}
fn matches_dir(rule: &Rule, dir_path: &str) -> bool {
    rel_under_base(rule, dir_path)
        .is_some_and(|path| pattern_matches(&rule.pattern, path, rule.dir_only))
}
...
fn glob_matches(pattern: &str, text: &str) -> bool {
    ...
    if let Some(prefix) = pat.strip_suffix('*') { return text.starts_with(prefix) || text.split('/').any(|seg| seg.starts_with(prefix)); }
    text == pat || text.starts_with(&format!("{pat}/"))
}
fn parent_dir_excluded(rel_str: &str, rules: &[Rule]) -> bool {
    let Some((parent, _)) = rel_str.rsplit_once('/') else { return false; };
    let mut prefix = String::new();
    for part in parent.split('/') {
        ...
        if dir_ignored(&prefix, rules) { return true; }
    }
    false
}
