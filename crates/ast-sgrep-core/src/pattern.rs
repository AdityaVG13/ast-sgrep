...
use ast_sgrep_lang::{detect_language, match_literal_pattern, needs_ast_grep_fallback};
use walkdir::WalkDir;
...
pub fn search_pattern(
    ...
    if !needs_ast_grep_fallback(pattern) {
        ...
        if !native.is_empty() || find_ast_grep_binary().is_none() { return Ok(native); }
    }
    search_pattern_ast_grep(pattern, root, lang_filter)
}
fn cached_pattern_signatures(pattern: &str) -> Option<Vec<String>> {
    let pattern = pattern.trim();
    if pattern.is_empty() { return Some(vec![]); }
    if !pattern.contains('$') { return Some(vec![pattern.to_string()]); }
    for (prefix, kind) in [("fn ", "function_item"), ("def ", "function_definition")] {
        if let Some(rest) = pattern.strip_prefix(prefix) {
            let name = rest.split(|ch: char| ch == '(' || ch.is_whitespace()).next().unwrap_or_default();
            if name.starts_with('$') { return Some(vec![format!("kind:{kind}")]); }
            if is_pattern_identifier(name) { return Some(vec![format!("decl:{}:{name}", prefix.trim())]); }
            return None;
        }
    }
    let open = pattern.find('(')?;
    let close = pattern.rfind(')')?;
    if close + 1 != pattern.len() || !pattern[open + 1..close].contains("$$$") { return None; }
    let callee = pattern[..open].trim();
    if callee.starts_with('$') && !callee.contains('.') { return Some(vec!["kind:call_expression".into(), "kind:call".into()]); }
...
fn search_pattern_native(pattern: &str, root: &Path, lang_filter: Option<&str>) -> Result<Vec<SearchHit>> {
        ...
        .filter(|e| e.file_type().is_file())
    {
        ...
        if let Ok(matches) = match_literal_pattern(lang, &content, pattern) {
...
fn search_pattern_ast_grep(pattern: &str, root: &Path, lang_filter: Option<&str>) -> Result<Vec<SearchHit>> {
    let Some(ast_grep) = find_ast_grep_binary() else { return Err(crate::StoreError::Other(
            "ast-grep not found: install from https://github.com/ast-grep/ast-grep".into(),
        )); };
    let root = root.canonicalize().unwrap_or_else(|_| root.to_path_buf());
    let mut cmd = Command::new(&ast_grep);
    cmd.arg("run").arg("--pattern").arg(pattern).arg("--json").arg(&root)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    if let Some(lang) = lang_filter {
        cmd.arg("--lang").arg(lang);
    }
    let mut child = cmd.spawn().map_err(|e| crate::StoreError::Other(format!("failed to run {ast_grep}: {e}")))?;
    ...
    let status = loop {
        match child.try_wait() {
            ...
            Ok(None) if Instant::now() >= deadline => {
                child.kill().ok();
                return Err(crate::StoreError::Other(format!("ast-grep timed out after {PATTERN_TIMEOUT_SECS}s")));
            }
    ...
    if let Some(mut out) = child.stdout.take() {
        out.read_to_end(&mut stdout)
            .map_err(|e| crate::StoreError::Other(format!("failed to read ast-grep stdout: {e}")))?;
    }
    if let Some(mut err) = child.stderr.take() {
        err.read_to_end(&mut stderr).ok();
    }
    if !status.success() && stdout.is_empty() { return Err(crate::StoreError::Other(format!("ast-grep failed: {}", String::from_utf8_lossy(&stderr)))); }
...
fn find_ast_grep_binary() -> Option<String> {
    for name in ["ast-grep", "sg"] {
        ...
        if output.status.success() && String::from_utf8_lossy(&output.stdout).contains("ast-grep") { return Some(name.into()); }
    }
    ...
    bundled.is_file().then(|| bundled.to_string_lossy().into_owned())
}
fn normalize_hit_path(file: &str, root: &Path) -> String {
    let p = Path::new(file);
    if let Ok(rel) = p.strip_prefix(root) { return rel.to_string_lossy().replace('\\', "/"); }
    if let Ok(canon) = p.canonicalize() {
        let croot = root.canonicalize().unwrap_or_else(|_| root.to_path_buf());
        if let Ok(rel) = canon.strip_prefix(&croot) { return rel.to_string_lossy().replace('\\', "/"); }
    }
    file.replace('\\', "/")
}
...
pub fn bench_ast_grep(pattern: &str, root: &Path, iterations: u32) -> Option<f64> {
    ...
    let root = root.canonicalize().unwrap_or_else(|_| root.to_path_buf()).to_string_lossy().into_owned();
...
pub fn ast_grep_pattern_for_query(query: &str) -> Option<String> {
    let q = query.trim();
    let q = q.strip_prefix("defs:").or_else(|| q.strip_prefix("callers:")).unwrap_or(q).trim();
