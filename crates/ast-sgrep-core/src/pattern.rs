...
pub fn search_pattern(
    pattern: &str, store: &crate::store::IndexStore, root: &Path, lang_filter: Option<&str>,
) -> Result<Vec<SearchHit>> {
    if store.pattern_node_count()? > 0 {
        if let Some(signatures) = cached_pattern_signatures(pattern) {
            return search_pattern_cached(pattern, &signatures, store, lang_filter);
        }
    } if !needs_ast_grep_fallback(pattern) {
        let native = search_pattern_native(pattern, root, lang_filter)?; if !native.is_empty() || find_ast_grep_binary().is_none() { return Ok(native); }
    } search_pattern_ast_grep(pattern, root, lang_filter)
    ...
    pattern: &str, signatures: &[String], store: &crate::store::IndexStore, lang_filter: Option<&str>,
) -> Result<Vec<SearchHit>> {
    let mut hits = Vec::new(); let mut seen = std::collections::HashSet::new(); for signature in signatures {
        for row in store.pattern_nodes_matching(signature, lang_filter)? {
            if !seen.insert((row.path.clone(), row.line_start, row.line_end)) { continue; } hits.push(SearchHit::span(SpanHitInput {
                kind: HitKind::Pattern, file: row.path, line_start: row.line_start, line_end: row.line_end, score: SCORE_PATTERN, excerpt: row.excerpt, symbol: Some(pattern.to_string()), language: row.language,
            }));
        }
        ...
        .filter_entry(|e| !should_skip_dir(e.path())).filter_map(|e| e.ok()) .filter(|e| e.file_type().is_file())
    {
        ...
        if lang_filter.is_some_and(|f| lang.as_str() != f) { continue; } if let Ok(matches) = match_literal_pattern(lang, &content, pattern) {
            hits.extend(matches.into_iter().map(|m| SearchHit::span(SpanHitInput {
                kind: HitKind::Pattern, file: rel_str.clone(), line_start: m.line_start, line_end: m.line_end, score: SCORE_PATTERN, excerpt: m.excerpt, symbol: Some(pattern.to_string()), language: Some(lang.as_str().to_string()),
            })));
        }
    } Ok(hits)
    ...
    let deadline = Instant::now() + Duration::from_secs(PATTERN_TIMEOUT_SECS); let status = loop {
        match child.try_wait() {
            Ok(Some(status)) => break status, Ok(None) if Instant::now() >= deadline => {
                child.kill().ok(); return Err(crate::StoreError::Other(format!("ast-grep timed out after {PATTERN_TIMEOUT_SECS}s")));
            } Ok(None) => thread::sleep(Duration::from_millis(50)), Err(e) => {
                child.kill().ok(); return Err(crate::StoreError::Other(format!("ast-grep wait failed: {e}")));
            }
        }
    }; let mut stdout = Vec::new(); let mut stderr = Vec::new(); if let Some(mut out) = child.stdout.take() {
        out.read_to_end(&mut stdout).map_err(|e| crate::StoreError::Other(format!("failed to read ast-grep stdout: {e}")))?;
    } if let Some(mut err) = child.stderr.take() { err.read_to_end(&mut stderr).ok(); } if !status.success() && stdout.is_empty() {
    ...
    for name in ["ast-grep", "sg"] {
        let Ok(output) = Command::new(name).arg("--version").output() else { continue }; if output.status.success() && String::from_utf8_lossy(&output.stdout).contains("ast-grep") { return Some(name.into()); }
    ...
    let p = Path::new(file); if let Ok(rel) = p.strip_prefix(root) { return rel.to_string_lossy().replace('\\', "/"); } if let Ok(canon) = p.canonicalize() {
        let croot = root.canonicalize().unwrap_or_else(|_| root.to_path_buf()); if let Ok(rel) = canon.strip_prefix(&croot) { return rel.to_string_lossy().replace('\\', "/"); }
    } file.replace('\\', "/")
    ...
    let q = query.trim(); let q = q.strip_prefix("defs:").or_else(|| q.strip_prefix("callers:")).unwrap_or(q).trim(); (!q.is_empty() && !q.contains(' ')).then(|| q.to_string())
}
