...
fn hit_symbol(store: &IndexStore, hit: &SearchHit) -> Option<(String, u32, u32)> {
    if let Some(ref sym) = hit.symbol { return Some((sym.clone(), hit.line_start, hit.line_end)); }
    if let Ok(Some(row)) = store.symbol_at_line(&hit.file, hit.line_start) { return Some((row.name, row.line_start, row.line_end)); }
...
fn build_seeds(store: &IndexStore, hits: &[SearchHit], top_n: usize) -> Vec<ChainEntry> {
    ...
    let mut sorted: Vec<_> = best_per_file.values().map(|(hit, _)| (hit.file.as_str(), *hit)).collect();
    sorted.sort_by(|a, b| {
        b.1.score
...
fn dedup_by_file(entries: &[ChainEntry]) -> Vec<ChainEntry> {
    ...
    for e in entries {
        match best.get(e.file.as_str()) {
            Some(&existing) if existing.score > e.score || (existing.score == e.score && existing.depth <= e.depth) => {}
            _ => {
...
pub fn expand_chain(store: &IndexStore, query: &str, config: &ChainConfig) -> Result<ChainResponse> {
    ...
    for d in 1..=config.max_depth {
        ...
        let novel: Vec<_> = dedup_by_file(&next).into_iter().filter(|e| seen_files.insert(e.file.clone())).collect();
    ...
    all_edges.retain(|e| node_files.contains(e.from_file.as_str()) && node_files.contains(e.to_file.as_str()));
