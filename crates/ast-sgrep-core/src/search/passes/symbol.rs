use crate::query::ParsedQuery; use crate::rank::{best_symbol_score, score_caller, score_def, SCORE_ANCHOR, SCORE_GRAPH}; use crate::search::hits::matches_lang; use crate::search::types::{HitKind, SearchHit, SearchOptions, SpanHitInput};
...
enum CallerMatchMode {
    ...
    store: &IndexStore, filter: CallerFilter, terms: &[String], lang_filter: Option<&str>, limit: usize,
) -> Result<Vec<CallerQueryRow>> {
    let (where_clause, bind) = filter(terms, lang_filter); let sql = format!("{CALLER_SELECT}{where_clause} LIMIT ?{}", bind.len() + 1); query_limit_map(store.connection(), &sql, bind, limit, |row| {
        Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?, row.get(5)?))
    })
} fn caller_rows_to_hits(
    rows: Vec<CallerQueryRow>, options: &SearchOptions, parsed: &ParsedQuery, mode: CallerMatchMode,
    ...
    let primary_lower = parsed.primary_symbol().map(|s| s.to_lowercase()); let mut hits = Vec::new(); for (path, language, caller, callee, line_no, text) in rows {
        if !matches_lang(language.as_deref(), options.lang_filter.as_deref()) { continue; } let callee_score = best_symbol_score(&parsed.terms, &callee); let matched = match mode {
            ...
            path.clone(), language.clone(), caller.clone(), callee.clone(), line_no, text, score_caller(&parsed.terms, &callee),
    ...
    Ok((
        row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?, row.get(4)?, row.get(5)?, row.get::<_, Option<String>>(8)?.unwrap_or_default(),
    ))
} fn query_symbol_spans(
    store: &IndexStore, where_clause: &str, bind: Vec<String>, limit: usize,
) -> Result<Vec<SymbolSpanRow>> {
    let sql = format!("{SYMBOL_SELECT}{where_clause}{SYMBOL_KIND_ORDER} LIMIT ?{}", bind.len() + 1); query_limit_map(store.connection(), &sql, bind, limit, read_symbol_span_row)
} fn kind_weight(kind: &str) -> f64 {
    match kind {
        "function" | "method" => 1.0, _ => TYPE_SYMBOL_WEIGHT,
    }
} fn symbol_span_rows_to_hits(
    rows: Vec<SymbolSpanRow>, options: &SearchOptions, kind: HitKind, score_for: impl Fn(&str) -> f64,
) -> Result<Vec<SearchHit>> {
    Ok(rows
        .into_iter() .filter(|(_, language, ..)| matches_lang(language.as_deref(), options.lang_filter.as_deref())) .map(|(path, language, name, sym_kind, line_start, line_end, text)| {
            SearchHit::span(SpanHitInput {
                kind, file: path, line_start, line_end, score: score_for(&name) * kind_weight(&sym_kind), excerpt: text, symbol: Some(name), language,
            })
        }) .collect())
} pub fn symbol_pass(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    let mut hits = def_hits_for_terms(store, options, parsed, SYMBOL_SQL_LIMIT)?; hits.extend(caller_hits_for_terms(store, options, parsed, CALLER_SQL_LIMIT)?); Ok(hits)
} pub fn anchor_pass(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    let anchor_symbol = match parsed.primary_symbol() {
        Some(s) => s.to_string(), None => parsed.terms.iter().find(|t| t.len() > 3).cloned().unwrap_or_default(),
    }; if anchor_symbol.is_empty() { return Ok(vec![]); } let terms = vec![anchor_symbol]; let (where_clause, bind) = like_terms_filter("s.name", &terms, options.lang_filter.as_deref());
    let rows = query_symbol_spans(store, &where_clause, bind, SYMBOL_SQL_LIMIT)?; let term_count = parsed.terms.len(); symbol_span_rows_to_hits(rows, options, HitKind::Anchor, |name| {
        let matched = parsed.terms.iter().filter(|t| crate::rank::score_symbol(t, name) > 0.0).count(); if matched == 0 {
            0.0
        } else {
    ...
    store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery, limit: usize,
) -> Result<Vec<SearchHit>> {
    ...
    let symbol = parsed.lookup_symbol(); (!symbol.is_empty()).then(|| ParsedQuery { terms: vec![symbol], ..parsed.clone() })
} pub fn search_callers(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
