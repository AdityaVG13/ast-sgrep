...
use crate::store::sql::{
    callee_terms_filter, caller_terms_filter, like_terms_filter, query_limit_map,
};
...
fn caller_rows_to_hits(
    ...
    for (path, language, caller, callee, line_no, text) in rows {
        ...
        let (callee_score, caller_score) = (
            best_symbol_score(&parsed.terms, &callee),
            best_symbol_score(&parsed.terms, &caller),
        );
        ...
        hits.push(SearchHit::caller(
            path.clone(), language.clone(), caller.clone(), callee.clone(), line_no, text,
        ...
        if let Some(graph_score) = graph {
            hits.push(SearchHit::graph_scored(path, language, caller, callee, line_no, graph_score));
        }
    }
    Ok(hits)
}
...
fn symbol_span_rows_to_hits(
    ...
    Ok(rows
        .into_iter()
        .filter(|(_, language, ..)| matches_lang(language.as_deref(), options.lang_filter.as_deref()))
        .map(|(path, language, name, sym_kind, line_start, line_end, text)| {
...
pub fn symbol_pass(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    ...
    hits.extend(caller_hits_for_terms(store, options, parsed, CALLER_SQL_LIMIT)?);
    Ok(hits)
}
pub fn anchor_pass(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    let anchor_symbol = match parsed.primary_symbol() {
        Some(s) => s.to_string(),
        None => parsed.terms.iter().find(|t| t.len() > 3).cloned().unwrap_or_default(),
    };
    if anchor_symbol.is_empty() { return Ok(vec![]); }
    let terms = vec![anchor_symbol];
    let (where_clause, bind) = like_terms_filter("s.name", &terms, options.lang_filter.as_deref());
    let sql = format!("{SYMBOL_SELECT}{where_clause}{SYMBOL_KIND_ORDER} LIMIT ?{}", bind.len() + 1);
    ...
    symbol_span_rows_to_hits(rows, options, HitKind::Anchor, |name| {
        let matched = parsed.terms.iter().filter(|t| crate::rank::score_symbol(t, name) > 0.0).count();
        if matched == 0 {
            0.0
        } else {
...
pub(crate) fn def_hits_for_terms(
    ...
    if parsed.terms.is_empty() { return Ok(vec![]); }
    let (where_clause, bind) = like_terms_filter("s.name", &parsed.terms, options.lang_filter.as_deref());
    let sql = format!("{SYMBOL_SELECT}{where_clause}{SYMBOL_KIND_ORDER} LIMIT ?{}", bind.len() + 1);
    let rows = query_limit_map(store.connection(), &sql, bind, limit, read_symbol_span_row)?;
    symbol_span_rows_to_hits(rows, options, HitKind::Def, |name| score_def(&parsed.terms, name))
}
pub(crate) fn caller_hits_for_terms(
    ...
    if parsed.terms.is_empty() { return Ok(vec![]); }
    caller_rows_to_hits(
        query_caller_rows(store, caller_terms_filter, &parsed.terms, options.lang_filter.as_deref(), limit)?,
        options,
        parsed,
        CallerMatchMode::Hybrid,
    )
...
pub(crate) fn callee_hits_for_terms(
    ...
    if parsed.terms.is_empty() { return Ok(vec![]); }
    caller_rows_to_hits(
        query_caller_rows(store, callee_terms_filter, &parsed.terms, options.lang_filter.as_deref(), limit)?,
        options,
        parsed,
...
fn prefixed_mode_query(parsed: &ParsedQuery) -> Option<ParsedQuery> {
    ...
    (!symbol.is_empty()).then(|| ParsedQuery { terms: vec![symbol], ..parsed.clone() })
}
pub fn search_callers(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    match prefixed_mode_query(parsed) {
        Some(q) => callee_hits_for_terms(store, options, &q, MODE_SQL_LIMIT),
        None => Ok(vec![]),
    }
}
pub fn search_defs(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    match prefixed_mode_query(parsed) {
        Some(q) => def_hits_for_terms(store, options, &q, MODE_SQL_LIMIT),
        None => Ok(vec![]),
    }
}
pub fn search_imports(store: &IndexStore, options: &SearchOptions, parsed: &ParsedQuery) -> Result<Vec<SearchHit>> {
    ...
    Ok(store
        ...
        .into_iter()
        .map(|(path, language, module_path, line_no)| SearchHit::import(path, language, module_path, line_no))
        .collect())
}
