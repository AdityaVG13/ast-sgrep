...
pub fn embed_pass_lazy_ivf(
    ...
    if parsed.terms.is_empty() || !options.use_embed { return Ok(Some(Vec::new())); }
    let stats = store.semantic_chunk_stats(options.lang_filter.as_deref())?;
    if !crate::semantic_ann::should_use_ann(stats.count, options.ann_threshold) || stats.dim == 0 { return Ok(None); }
    let backend = store
    ...
    if ivf.chunk_count() != stats.count || ivf.dim != stats.dim { return Ok(None); }
    ...
    if candidate_indices.is_empty() { return Ok(None); }
    let ids = store.semantic_chunk_ids(options.lang_filter.as_deref())?;
    if ids.len() != stats.count { return Ok(None); }
    ...
    for id in candidate_ids {
        let Some(row) = rows.remove(&id) else { return Ok(None); };
        chunks.push(row);
    }
    let ranked =
...
pub fn embed_pass_with_context(
    ...
    if parsed.terms.is_empty() || !options.use_embed { return Ok(Vec::new()); }
    let query = parsed.terms.join(" ");
    let owned;
