...
pub fn literal_pass(
    ...
    if store.indexed_line_count()? >= BMH_LINE_THRESHOLD && needle.chars().count() >= 3 {
...
fn literal_trigram(
    ...
    for row in rows {
        ...
        let excerpt_text = excerpt(file_map.as_ref(), &path, line_no, content, options);
        hits.push(span_hit(path, language, line_no, excerpt_text, 1.0));
    }
    Ok(hits)
}
fn literal_sql(
    ...
    let sql = format!(
        "SELECT f.path, f.language, l.line_no, l.content
         ...
         WHERE l.content {op} ?1 ORDER BY f.path, l.line_no LIMIT ?2"
    );
    ...
    for (rank, row) in rows.enumerate() {
        ...
        hits.push(span_hit(
            ...
            language,
            line_no,
            excerpt(file_map.as_ref(), &path, line_no, content, options),
            1.0 / (1.0 + rank as f64 * 0.01),
        ));
    }
    Ok(hits)
}
fn needs_context(options: &SearchOptions) -> bool {
    options.context_before > 0 || options.context_after > 0
}
fn map_line_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<(String, Option<String>, u32, String)> {
...
fn has_literal_match(haystack: &str, needle: &str, word_mode: bool) -> bool {
    if !word_mode { return haystack.contains(needle); }
    haystack
