...
pub struct CliSession { pub _temp: TempDir, pub root: PathBuf, pub index_path: PathBuf, pub bin: PathBuf }
impl CliSession {
    pub fn sample(bin: PathBuf) -> Self {
        let temp = TempDir::new().expect("tempdir");
        let session = Self { root: sample_root(), index_path: temp.path().join("index.db"), bin, _temp: temp };
    ...
    pub fn search_json(&self, query: &str, extra: &[&str]) -> Value {
        ...
        args.extend(extra);
        if !query.is_empty() { args.push(query); }
    ...
    pub fn run(&self, args: &[&str]) -> Result<Output, String> { Command::new(&self.bin).args(args).output().map_err(|e| e.to_string()) }
    fn index(&self) -> Result<Output, String> { self.run(&["--index-path", self.index_path.to_str().unwrap(), "index", self.root.to_str().unwrap()]) }
}
