...
pub fn lsp_search_hit_keys(root: &Path, index_path: &Path, query: &str, limit: usize) -> Vec<HitKey> {
    let mut backend = LspBackend::new(root.to_path_buf());
    backend.set_index_path(index_path.to_path_buf());
    backend.apply_settings(AsgrepSettings { no_embed: Some(true), ..AsgrepSettings::default() });
