...
pub struct IndexedFixture { pub _temp: TempDir, pub indexer: Indexer }
...
pub fn index_sample(mut opts: IndexOptions) -> IndexedFixture {
    ...
    if opts.root.as_os_str() == "." { opts.root = sample_root(); }
    let mut indexer = Indexer::new(opts).expect("indexer");
    indexer.index_all().expect("index");
    IndexedFixture { _temp: temp, indexer }
}
...
pub struct HitKey { pub file: String, pub line_start: u32, pub kind: String }
pub fn response_hit_keys(response: &SearchResponse) -> Vec<HitKey> {
    response.hits.iter().map(|hit| HitKey { file: hit.file.clone(), line_start: hit.line_start, kind: hit.kind.as_str().to_owned(), }).collect()
}
pub fn json_hit_keys(response: &Value) -> Vec<HitKey> {
    response["hits"].as_array().expect("search response hits").iter().map(|hit| HitKey {
        file: hit["file"].as_str().expect("hit file").to_owned(), line_start: hit["line_start"].as_u64().expect("hit line_start") as u32,
        kind: hit["kind"].as_str().expect("hit kind").to_owned(), }).collect()
}
pub fn core_search_hit_keys(root: &Path, index_path: &Path, query: &str, limit: usize) -> Vec<HitKey> {
