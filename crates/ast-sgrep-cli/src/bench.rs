...
pub(crate) fn maybe_index(
    root: &Path,
    cli: &Cli,
    skip: bool,
) -> anyhow::Result<(Option<IndexStats>, f64)> {
    if skip {
        return Ok((None, 0.0));
    }
...
pub(crate) fn bench_searcher(root: &Path, cli: &Cli, skip_index: bool) -> anyhow::Result<Searcher> {
...
pub(crate) fn print_index_skipped(stats: Option<&IndexStats>, index_ms: Option<f64>) {
