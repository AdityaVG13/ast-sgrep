...
/// A quarantine (too noisy to judge) warns by default so bench stays usable
...
pub fn enforce_bench_ratchet(
    ...
    if crate::keep_gate::bench_ratchet_enabled() && h["ratchet_ok"] == false {
        if h["verdict"] == "quarantine_cv" && !crate::keep_gate::bench_strict_enabled() {
            eprintln!(
                "keep-gate quarantine for {subject}: verdict=quarantine_cv cv_pct={:?} — \
                 measurements recorded but not blessed (noisy host); \
                 set ASGREP_BENCH_STRICT=1 to fail hard",
                h.get("cv_pct"),
            );
            return Ok(());
        }
