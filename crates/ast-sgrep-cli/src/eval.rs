...
fn run_single(
    ...
    let evals = gold
        ...
        .map(|q| {
            ...
            Ok(evaluate_query(q, &response.hits))
        })
...
fn query_eval_json(e: &QueryEval) -> Value {
    ...
    json!({
        ...
        "found": e.found, "relevant": e.relevant, "ndcg": round3(e.ndcg),
        "recall_at": Value::Object(recall_at),
    })
}
...
fn single_json(
    ...
    json!({
        "gold": gold_path.display().to_string(),
        "corpus": gold.corpus,
        "config": cfg.json(root, index_path),
        "queries": evals.iter().map(query_eval_json).collect::<Vec<_>>(),
...
fn print_ab(
    ...
    if cli.json {
        ...
        let queries: Vec<Value> = evals_a
            .iter()
            .zip(evals_b.iter())
            .map(|(qa, qb)| {
                json!({
                    "name": qa.name, "rank_a": qa.first_rank, "rank_b": qb.first_rank,
                    "delta_rr": round3(qb.rr - qa.rr), "delta_ndcg": round3(qb.ndcg - qa.ndcg),
                })
            })
            .collect();
        let aggregate = json!({
            "delta_mrr": round3(agg_b.mrr - agg_a.mrr),
            "delta_ndcg": round3(agg_b.ndcg - agg_a.ndcg),
            "delta_recall_at_k": round3(agg_b.recall_at_k - agg_a.recall_at_k),
            "delta_recall_at_1": round3(agg_b.recall_at[0].1 - agg_a.recall_at[0].1),
    ...
    println!();
    println!(
        "delta MRR={:+.3}  delta Recall@k={:+.3}  delta nDCG@k={:+.3}  delta Recall@1={:+.3}  delta Recall@5={:+.3}  delta Recall@20={:+.3}",
        agg_b.mrr - agg_a.mrr, agg_b.recall_at_k - agg_a.recall_at_k, agg_b.ndcg - agg_a.ndcg,
