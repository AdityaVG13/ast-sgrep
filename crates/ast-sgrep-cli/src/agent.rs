...
pub(crate) fn capabilities_json(_cli: &Cli) -> anyhow::Result<Value> {
    Ok(json!({
        ...
        "agent_contract": {
            "stdout": "data payloads only when --json / robot modes are set",
            "stderr": "human hints and diagnostics",
        ...
        "global_flags": [
            "--json", "--root", "--limit", "--index-path", "--lang", "--format",
            "--no-embed", "--tantivy", "--ann-threshold"
        ],
        "environment": [
            "ASGREP_LIMIT", "ASGREP_INDEX_PATH", "ASGREP_NO_EMBED", "ASGREP_CLOUD_EMBED",
        ...
        "exit_codes": [
            ...
            {"code": 2, "meaning": "index or search operation failed"},
        ],
        "canonical_tasks": [
            "asgrep index . && asgrep --json --format agent \"where is auth refreshed\" .",
            "asgrep status . --json",
            "asgrep doctor . --robot-triage"
        ],
    }))
}
fn doctor_triage_json(cli: &Cli, root: &Path) -> anyhow::Result<Value> {
    ...
    if status.is_none() {
        ...
        if !st.semantic_ivf_present && st.semantic_chunk_count > 0 {
            issues.push(json!({
                "kind": "semantic_ivf_missing",
                "message": "semantic chunks present but IVF sidecar not built (may be below ANN threshold)"
            }));
        }
    }
    if next.is_empty() {
