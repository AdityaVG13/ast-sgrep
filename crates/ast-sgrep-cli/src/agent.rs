use crate::{index_options, Cli}; use anyhow::Context; use ast_sgrep_core::Indexer; use clap::{Parser, Subcommand};
use serde_json::{json, Value}; use std::io::{self, IsTerminal}; use std::path::Path;
const TOOL: &str = "asgrep"; const SCHEMA: &str = "1.0.0";
#[derive(Parser)] pub(crate) struct CapabilitiesArgs { #[arg(long)] pub(crate) json: bool }
#[derive(Subcommand)] pub(crate) enum RobotDocsCommand { Guide }
#[derive(Parser)] pub(crate) struct RobotDocsArgs { #[command(subcommand)] command: RobotDocsCommand }
#[derive(Parser)] pub(crate) struct DoctorArgs { #[arg(long)] pub(crate) json: bool, #[arg(long = "robot-triage")] pub(crate) robot_triage: bool }
pub(crate) fn run_capabilities(cli: &Cli, args: &CapabilitiesArgs) -> anyhow::Result<()> {
    if !cli.json && !args.json {
        ...
        return Err(crate::usage_error("capabilities requires --json for deterministic agent output"));
    }
...
pub(crate) fn run_robot_docs(_cli: &Cli, args: &RobotDocsArgs) -> anyhow::Result<()> {
    match args.command { RobotDocsCommand::Guide => { print_robot_guide(); Ok(()) } }
}
pub(crate) fn run_doctor(cli: &Cli, root: &Path, args: &DoctorArgs) -> anyhow::Result<()> {
    if cli.json || args.robot_triage || args.json { return crate::print_machine_json("doctor", doctor_triage_json(cli, root)?); }
    eprintln!("hint: use `{TOOL} doctor --json` or `{TOOL} doctor --robot-triage` for agent-readable triage");
    Err(crate::usage_error("doctor requires --json or --robot-triage"))
}
...
fn doctor_triage_json(cli: &Cli, root: &Path) -> anyhow::Result<Value> {
    let root = crate::effective_root(cli, root); let mut issues = Vec::<Value>::new(); let mut next = Vec::<&'static str>::new();
    let status = match Indexer::new(index_options(&root, cli)).context("failed to open index for doctor") {
        Ok(idx) => match idx.store().status() { Ok(status) => Some(status), Err(e) => { issues.push(json!({"kind": "status_read", "message": e.to_string()})); None } },
        Err(e) => { issues.push(json!({"kind": "index_open", "message": e.to_string()})); None }
    };
    if status.is_none() { next.push("asgrep index . --json"); }
    else if let Some(ref st) = status {
        if st.file_count == 0 { issues.push(json!({"kind": "empty_index", "message": "index exists but indexes zero files"})); next.push("asgrep index . --json"); }
    ...
    if next.is_empty() { next.push("asgrep --json --format agent \"<your query>\" ."); }
    next.extend(["asgrep capabilities --json", "asgrep robot-docs guide"]);
    Ok(json!({"schema_version": SCHEMA, "tool": TOOL, "robot_triage": true, "root": root, "index_path": cli.index_path, "status": status, "issues": issues, "suggested_commands": next, "healthy": issues.is_empty(), "tty": io::stdout().is_terminal()}))
}
fn print_robot_guide() {
    print!(r#"# asgrep — agent handbook (robot-docs guide)
## Quick start
...
pub(crate) fn query_looks_like_subcommand_typo(query: &str) -> Option<&'static str> {
    let q = query.trim(); if q.is_empty() || q.contains(' ') { return None; }
    let lower = q.to_ascii_lowercase();
    const EXACT: &[&str] = &["index","status","reindex","bench","watch","semantic","capabilities","capability","robot-docs","robot_docs","doctor","help"];
    if let Some(c) = EXACT.iter().find(|c| lower == **c) { return Some(*c); }
    const TYPOS: &[(&str, &str)] = &[("capabilites","capabilities"),("capabilty","capabilities"),("statu","status"),("indx","index"),("reindx","reindex"),("sematic","semantic"),("doctr","doctor")];
...
pub(crate) fn print_agent_help_footer() {
    ...
    eprintln!("Exit codes: 0=ok, 1=usage, 2=operation failed. Use --json for machine-readable stdout.");
}
