...
/// Strict ratchet: a quarantined (too-noisy-to-judge) run fails the command
/// instead of warning. Default is warn-and-continue so `asgrep bench` stays
/// usable as a measurement tool on noisy hosts; quarantine still refuses to
/// bless or record a baseline either way.
pub fn bench_strict_enabled() -> bool {
    matches!(
        std::env::var("ASGREP_BENCH_STRICT").ok().as_deref(),
...
mod tests {
    ...
    fn verdicts_quarantine_reject_baseline_keep() {
        ...
        assert!(matches!(
            evaluate_keep(
                KeepSample { avg_ms: 1.0, cv_pct: 926.0, geomean_ms: None },
                prior(10.0),
                t
            ),
        ...
        assert!(matches!(
            evaluate_keep(
                KeepSample { avg_ms: 50.0, cv_pct: 1.0, geomean_ms: None },
                KeepPrior { avg_ms: None, geomean_ms: None, placeholder: true },
                t
            ),
        ...
        assert!(matches!(
            evaluate_keep(
                KeepSample { avg_ms: 11.0, cv_pct: 1.0, geomean_ms: None },
                prior(10.0),
                t
            ),
            KeepVerdict::RejectRegression { kind: "primary", .. }
        ));
        // +2% keeps.
        assert!(matches!(
            evaluate_keep(
                KeepSample { avg_ms: 10.2, cv_pct: 1.0, geomean_ms: None },
                prior(10.0),
                t
            ),
            KeepVerdict::Keep { .. }
        ...
        assert!(matches!(
            evaluate_keep(
                KeepSample { avg_ms: 10.2, cv_pct: 1.0, geomean_ms: Some(10.6) },
                KeepPrior { avg_ms: Some(10.0), geomean_ms: Some(10.0), placeholder: false },
                t
            ),
            KeepVerdict::RejectRegression { kind: "geomean", .. }
        ));
    }
}
