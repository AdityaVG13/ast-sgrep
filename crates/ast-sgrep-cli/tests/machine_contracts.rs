...
#[test]
fn neural_embed_and_rerank_fail_closed_without_features() {
    let bin = asgrep_bin();
    #[cfg(not(feature = "neural-embed"))]
    {
        let output = run(&bin, &["--json", "--neural-embed", "status", "."]);
    ...
    {
        let output = run(&bin, &["--json", "--rerank=true", "status", "."]);
        assert_eq!(output.status.code(), Some(2));
        let value = parse_stdout(&output);
        assert_eq!(value["ok"], false);
        assert!(
...
fn bench_suite_json_is_single_envelope_even_on_failure() {
    ...
    let output = Command::new(&session.bin)
        ...
        .output()
        .expect("bench suite");
    // stdout must parse as exactly one JSON value (parse_stdout already enforces this).
    let value = parse_stdout(&output);
    ...
    // On success ok matches suite_ok; dual envelopes would fail parse_stdout.
