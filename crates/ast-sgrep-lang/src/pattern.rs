...
pub fn match_literal_pattern(
    ...
    walk_pattern_nodes(tree.root_node(), source, pattern, &mut matches);
    Ok(matches)
}
fn walk_pattern_nodes(node: Node, source: &str, pattern: &str, out: &mut Vec<PatternMatch>) {
    ...
    for child in node.children(&mut cursor) {
        walk_pattern_nodes(child, source, pattern, out);
    }
}
...
fn declaration_prefix(kind: &str) -> Option<&'static str> {
    match kind {
        ...
        "function_declaration" | "method_definition" | "method_declaration" => Some("function"),
