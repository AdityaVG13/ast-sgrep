use tree_sitter::{Node, Parser}; use crate::extract::{byte_to_line, is_in_comment_or_string, node_lines, node_text}; use crate::{Language, PatternNode}; #[derive(Debug, Clone, PartialEq, Eq)] pub struct PatternMatch {
    pub line_start: u32, pub line_end: u32, pub excerpt: String,
} pub fn needs_ast_grep_fallback(pattern: &str) -> bool { pattern.contains('$') } pub fn tree_sitter_language(lang: Language) -> tree_sitter::Language {
    match lang {
...
pub fn match_literal_pattern(
    lang: Language, source: &str, pattern: &str,
    ...
    if !is_in_comment_or_string(&node) {
        if identifier_matches(&node, source, pattern) { push_match(&node, source, pattern, out); } if let Some(name_node) = node.child_by_field_name("name") {
            if identifier_matches(&name_node, source, pattern) { push_match(&node, source, pattern, out); }
        }
    ...
    let mut out = Vec::new(); let mut seen = std::collections::HashSet::new(); collect_node_signatures(root, source, &mut out, &mut seen); out
} fn collect_node_signatures(
    node: Node, source: &str, out: &mut Vec<PatternNode>, seen: &mut std::collections::HashSet<(String, u32)>,
) {
    if !is_in_comment_or_string(&node) {
        if is_identifier_kind(node.kind()) {
            if let Some(text) = node_text(&node, source) { push_pattern_node(node, source, text, out, seen); }
            ...
            push_pattern_node(node, source, &format!("kind:{}", node.kind()), out, seen); if let Some(callee) = call_target(&node, source) {
                push_pattern_node(node, source, &format!("call:{callee}"), out, seen); if let Some(name) = callee.rsplit(['.', ':']).find(|p| !p.is_empty()) {
    ...
    node: Node, source: &str, signature: &str, out: &mut Vec<PatternNode>, seen: &mut std::collections::HashSet<(String, u32)>,
) {
    let (line_start, line_end) = node_lines(&node, source); if !seen.insert((signature.to_string(), line_start)) { return; } out.push(PatternNode {
        signature: signature.to_string(), line_start, line_end, excerpt: excerpt_for_node(&node, source, signature),
    });
    ...
    if let Some(text) = node_text(node, source) {
        if text.lines().count() <= 6 { return text.to_string(); }
    } let line = byte_to_line(source, node.start_byte()); source
