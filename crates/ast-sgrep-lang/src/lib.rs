...
pub fn detect_language(path: &Path, content: Option<&str>) -> Option<Language> {
    ...
    if trimmed.starts_with("package ") { return Some(Language::Go); }
    if trimmed.starts_with("#!/usr/bin/env ruby") || trimmed.starts_with("#!/usr/bin/ruby") { return Some(Language::Ruby); }
    if trimmed.starts_with("#!/usr/bin/env python") || trimmed.starts_with("#!/usr/bin/python") { return Some(Language::Python); }
    None
}
