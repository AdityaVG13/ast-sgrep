...
parser!(
    ...
    |ext, node, source| match node.kind() {
        "function_item" => add_named_symbol(
            ext,
            node,
            source,
            if is_inside_kind(node, "impl_item") {
                SymbolKind::Method
            } else {
                SymbolKind::Function
            },
        ),
        "struct_item" | "union_item" | "type_item" =>
            add_named_symbol(ext, node, source, SymbolKind::Type),
...
parser!(
    ...
    |ext, node, source| match node.kind() {
        "function_definition" => add_named_symbol(
            ...
            node,
            source,
            if is_inside_kind(node, "class_definition") {
                SymbolKind::Method
            } else {
                SymbolKind::Function
            },
...
parser!(
    ...
    |ext, node, source| match node.kind() {
        "method_declaration" | "constructor_declaration" => add_named_symbol(
            ...
            node,
            source,
            if is_inside_kind(node, "class_declaration") {
                SymbolKind::Method
            } else {
                SymbolKind::Function
            },
...
parser!(
    ...
    |ext, node, source| match node.kind() {
        "method_declaration" | "constructor_declaration" => add_named_symbol(
            ...
            node,
            source,
            if inside_type_declaration(node) {
                SymbolKind::Method
            } else {
                SymbolKind::Function
            },
...
fn csharp_using_path(node: &tree_sitter::Node, source: &str) -> Option<String> {
    ...
    for child in node.children(&mut cursor) {
        if matches!(child.kind(), "qualified_name" | "identifier") {
            if let Some(text) = node_text(&child, source) {
                if text != "using" && text != "static" && text != "global" {
                    return Some(text.to_string());
                }
            }
        }
