...
parser!(RustParser, Rust, tree_sitter_rust::LANGUAGE, |ext, node, source| match node.kind() {
    "function_item" => add_named_symbol(
        ext, node, source,
        if is_inside_kind(node, "impl_item") { SymbolKind::Method } else { SymbolKind::Function },
    ),
    "struct_item" | "union_item" | "type_item" => add_named_symbol(ext, node, source, SymbolKind::Type),
    "enum_item" => add_named_symbol(ext, node, source, SymbolKind::Enum),
    "trait_item" => add_named_symbol(ext, node, source, SymbolKind::Interface),
    "call_expression" => {
...
parser!(PythonParser, Python, tree_sitter_python::LANGUAGE, |ext, node, source| match node.kind() {
    "function_definition" => add_named_symbol(
        ext, node, source,
        if is_inside_kind(node, "class_definition") { SymbolKind::Method } else { SymbolKind::Function },
    ),
    "class_definition" => add_named_symbol(ext, node, source, SymbolKind::Class),
    "call" => {
        if let Some(func) = field_child(node, "function") {
            ext.add_call(node, source, &func);
        }
...
parser!(GoParser, Go, tree_sitter_go::LANGUAGE, |ext, node, source| match node.kind() {
    "function_declaration" => add_named_symbol(ext, node, source, SymbolKind::Function),
    "method_declaration" => add_named_symbol(ext, node, source, SymbolKind::Method),
    "type_spec" => {
...
parser!(JavaParser, Java, tree_sitter_java::LANGUAGE, |ext, node, source| match node.kind() {
    "method_declaration" | "constructor_declaration" => add_named_symbol(
        ext, node, source,
        if is_inside_kind(node, "class_declaration") { SymbolKind::Method } else { SymbolKind::Function },
    ),
    "class_declaration" | "record_declaration" => add_named_symbol(ext, node, source, SymbolKind::Class),
    "interface_declaration" => add_named_symbol(ext, node, source, SymbolKind::Interface),
    "enum_declaration" => add_named_symbol(ext, node, source, SymbolKind::Enum),
    "field_declaration" => {
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
...
parser!(CSharpParser, CSharp, tree_sitter_c_sharp::LANGUAGE, |ext, node, source| match node.kind() {
    "method_declaration" | "constructor_declaration" => add_named_symbol(
        ext, node, source,
        if inside_type_declaration(node) { SymbolKind::Method } else { SymbolKind::Function },
    ),
    "local_function_statement" => add_named_symbol(ext, node, source, SymbolKind::Function),
    "class_declaration" | "record_declaration" => add_named_symbol(ext, node, source, SymbolKind::Class),
    "struct_declaration" => add_named_symbol(ext, node, source, SymbolKind::Type),
    "interface_declaration" => add_named_symbol(ext, node, source, SymbolKind::Interface),
    "enum_declaration" => add_named_symbol(ext, node, source, SymbolKind::Enum),
    "invocation_expression" => {
        if let Some(function) = field_child(node, "function") {
            ext.add_call(node, source, &function);
        }
    }
    "using_directive" => {
        if let Some(path) = csharp_using_path(node, source) {
            ext.add_import(node, source, &path);
        }
    }
    _ => {}
...
parser!(RubyParser, Ruby, tree_sitter_ruby::LANGUAGE, |ext, node, source| match node.kind() {
    "method" => add_named_symbol(
        ext, node, source,
        if is_inside_kind(node, "class") { SymbolKind::Method } else { SymbolKind::Function },
    ),
    "class" => add_named_symbol(ext, node, source, SymbolKind::Class),
    "module" => add_named_symbol(ext, node, source, SymbolKind::Type),
    "call" => {
    ...
    match node.kind() {
        ...
        _ => {
            ...
            for c in node.children(&mut cursor) {
                if let Some(s) = extract_ruby_string(&c, source) {
                    found = Some(s);
                    break;
                }
            }
...
fn handle_ts_js_node(ext: &mut crate::extract::Extractor, node: &tree_sitter::Node, source: &str) {
    match node.kind() {
        ...
        "method_definition" => add_named_symbol(ext, node, source, SymbolKind::Method),
        "class_declaration" | "abstract_class_declaration" => {
            add_named_symbol(ext, node, source, SymbolKind::Class)
        }
        "interface_declaration" => add_named_symbol(ext, node, source, SymbolKind::Interface),
        "type_alias_declaration" => add_named_symbol(ext, node, source, SymbolKind::Type),
        "enum_declaration" => add_named_symbol(ext, node, source, SymbolKind::Enum),
