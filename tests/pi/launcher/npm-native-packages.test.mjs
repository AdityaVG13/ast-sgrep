...
import { resolveBinary, resolveCodemodeAddon } from "../../../packages/pi/launcher/src/index.js";
...
test("npm omits a wrong-OS local optional package without registry access", () => {
  ...
  try {
    ...
    mkdirSync(nativeDir);
    mkdirSync(appDir);
    writeFileSync(join(nativeDir, "package.json"), JSON.stringify({ name: "@ast-sgrep/win32-x64-msvc", version: "1.4.0", os: ["win32"], cpu: ["x64"] }));
