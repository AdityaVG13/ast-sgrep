...
<!-- pass 7, 2026-09-21: ceiling-analysis rejection, no experiments (pass-4/6 precedent); 3rd consecutive reject = loop convergence -->
- **Skill-loop CONVERGED 2026-09-21 (pass 7).** Third consecutive ceiling reject (passes 5, 6, 7). Three blocks agree the largest shippable single-mechanism save is idx_lexicon_related ≈ 0.33ms vs an S1 bar of 0.51-0.71ms. The only pool that gates is the contract-blocked literal-expansion-skip (0.60ms = 11.8% S1 / 10.3% avg — recomputed at the post-pass-3 baseline). Further passes under current rules are expected to reject. Unlocks, in leverage order: (a) product decision on `query_expansions` for literal queries (pass-1 predicate); (b) a pre-main mechanism (research, nothing priced >0.2 today); (c) recalibration — ship the micro backlog (idx + manifest ≈ 0.5ms ≈ 9% S1) as plain micros with 4/4 goldens and no A/B bar, or restate the loop goal.
- **Spot-verification 2026-09-21 (skill-loop pass 7, own receipts).** Expansions pool REPLICATED: fenced_expansions 0.60 / lex_sql 0.56 (n=9 medians, 2 rounds) vs pass-6 0.62-0.65 / 0.59-0.62. Pre-main shape replicated but absolute smaller: pre 2.4-2.6 constant (vs 3.3-3.4), true 1.50 (vs 1.71) → asgrep-attributable ≈ 1.0-1.3ms (vs ~2.0ms); discrepancy owned as loaded-host spawn jitter — rusage replicates pass 6 almost exactly (S1 minflt 908 vs 910, ver 456 vs 451). Mechanism-free at either value; verdict unaffected.
- **libiconv LC_LOAD source RESOLVED 2026-09-21 (skill-loop pass 7).** The load comes from the `libc` crate's unconditional `#[link(name = "iconv")]` (src/unix/bsd/apple/mod.rs:5164; std builds dodge it via `rustc-dep-of-std`, which is why hello-world links libSystem only). Zero undefined iconv refs (nm); the dylib is shared-cache-resident, so `-Wl,-dead_strip_dylibs` would save tens of µs at most — priced WITHOUT building (a build to prove a null on a ≤0.2ms pool is process, not evidence). A libc 0.2→1.x upgrade would drop the link but is a tree-wide migration for the same ≤0.2 pool. Do opportunistically or never. METHOD NOTE: DYLD_PRINT_STATISTICS is silent on macOS 26 (stderr and _FILE variants) — xctrace remains the only dyld-visibility tool.
- **Schema slimming PRICED 2026-09-21 (skill-loop pass 7, stays closed).** Fixture schema = 47 objects / 5,098 bytes DDL; config_conn 0.29ms. Even halving the schema saves ≤ ~0.15ms and needs a multi-DB migration plus a product decision on DB topology. Below gate three ways (size, mechanism, migration cost). Retry predicate: re-open only with a product-approved multi-DB design whose RO-query schema touch is measured (wall-phase) ≥0.3ms cheaper with 4/4 goldens.
