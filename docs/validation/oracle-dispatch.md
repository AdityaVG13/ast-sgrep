...
This file is the router. It does **not** implement pattern×ast-grep (`ghiw.3`),
jell, or MUST matrices (`ghiw.2`). DISC ids come from
...
| structural-native | vs ast-grep CLI | excluded until ghiw.3 | `asgrep` | `ast-grep` CLI | match-set differential | `DISC-pattern-native-subset` | **ghiw.3** (not this bead) | `deferred_excluded` |
