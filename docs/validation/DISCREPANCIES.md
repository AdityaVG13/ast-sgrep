...
|---|---|---|---|---|
| `DISC-pattern-native-subset` | `pattern:` | Native tree-sitter + indexed signatures only. Nested templates, YAML rules, rewrites, and relational metavars return no hits. **No silent ast-grep subprocess.** | `docs/structural-patterns.md` | Fail if a test assumes ast-grep CLI. Ignore only with this DISC. |
