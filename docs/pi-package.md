...
This is the canonical package-user guide for the `1.3.1` contract. npm availability is established only by an authorized release, not by this repository documentation. For a project-local Pi installation, add `-l` to Pi package-management commands.
...
The npm layers are exact-version matched: the `pi-ast-sgrep` extension depends on `ast-sgrep`, which selects one of five host-constrained native packages. Extension, launcher, and native package manifests must all be `1.3.1`. The embedded executable reports native CLI version `1.3.1`; the runtime verifies that identity separately from the npm package version.
...
Pi release validation does not run automatically on pull requests, pushes to `main`, or tag pushes. Both Pi workflows are manual `workflow_dispatch` actions. Manually dispatch **Pi native artifacts** (`.github/workflows/pi-native-artifacts.yml`) for a safe dry-run that packs and tests without publishing. An official Pi/npm release is one human-approved `v1.3.1` tag and commit for the five native npm packages, launcher, and extension. Its contract separately pins the embedded native CLI at `1.3.1`. The `Pi npm official release` workflow must be dispatched against that exact tag with `publish=true`.
